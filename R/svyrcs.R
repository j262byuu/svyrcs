...
#'     \item{measure}{One of `"HR"` (Cox), `"OR"` (logit link), `"RR"` (log link on a binomial
#'       family), `"Rate ratio"` (log link on a count family with an offset), `"Mean ratio"` (log
#'       link on a count family without one), `"Ratio"` (log link on any other family), or
#'       `"Difference"` (identity link). A non-standard link gives `"<link>-scale difference"`.}
...
#'
#' Rubin's rules require a *common* parameterisation, not any particular way of arriving at one. This
#' package averages the survey-weighted quantiles across imputations, which is a convention rather
#' than a derivation; the average of quantiles is not in general the quantile of a pooled predictive
#' distribution, and for strongly skewed imputations another rule could reasonably be preferred.
#'
...
#' the package warns and substitutes the complete-data degrees of freedom. That substitution is not a
#' validated rule and should not be read as conservative: with a large fraction of missing
#' information the complete-data value can exceed an appropriate imputation-limited denominator.
#' Raising the number of imputations is the real fix.
#'
