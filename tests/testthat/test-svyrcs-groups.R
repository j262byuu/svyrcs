...
test_that("grouped fits work for every model family", {
  ...
  cases <- list(
    list(f = survival::Surv(time, event) ~ rcspline(bmi, 4) * sex + age, family = NULL, measure = "HR"),
    list(f = tchol ~ rcspline(bmi, 4) * sex + age, family = NULL, measure = "Difference"),
    list(f = high_chol ~ rcspline(bmi, 4) * sex + age, family = quasibinomial(), measure = "OR"),
    list(f = statin_use ~ rcspline(bmi, 4) * sex + age, family = quasipoisson(),
         measure = "Mean ratio")
  )
  for (cs in cases) {
...
test_that("a four-level modifier gives four curves and nine interaction df", {
  fit <- svyrcs(tchol ~ rcspline(bmi, 4) * race + age, design = nhanes_design(), n = 50)
...
test_that("the reference is shared and every group passes through the null there", {
  fit <- svyrcs(tchol ~ rcspline(bmi, 4) * race + age, design = nhanes_design())
...
test_that("a subset design keeps its own degrees of freedom", {
  ...
  fit <- svyrcs(tchol ~ rcspline(bmi, 4) * sex + age, design = sub)
...
test_that("rows with a missing modifier are dropped and counted", {
  ...
  fit <- svyrcs(bmi ~ rcspline(age, 4) * grp, design = design)
...
test_that("a rank-deficient interaction is diagnosed rather than reported as a missing column", {
  ...
  expect_error(svyrcs(bmi ~ rcspline(age, 4) * old, design = design), class = "svyrcs_error")
  expect_error(svyrcs(bmi ~ rcspline(age, 4) * old, design = design), "rank deficient")
})

test_that("misuse of the interaction form is rejected", {
  design <- nhanes_design()
  expect_error(svyrcs(tchol ~ rcspline(bmi, 4) * age, design = design), class = "svyrcs_error")
  expect_error(svyrcs(tchol ~ rcspline(bmi, 4) * sex * race, design = design), class = "svyrcs_error")
  expect_error(svyrcs(tchol ~ rcspline(bmi, 4):sex + sex, design = design), class = "svyrcs_error")
})

test_that("weighted knots are still shared across groups", {
  fit <- svyrcs(tchol ~ rcspline(bmi, 4) * sex + age, design = nhanes_design())
  plain <- svyrcs(tchol ~ rcspline(bmi, 4) + age, design = nhanes_design())
