...
test_that("a knot count and the equivalent explicit knots give the same basis", {
  ...
  x <- rnorm(200, 25, 5)
  for (nk in 3:7) {
    expect_equal(bare(rcs(x, nk)), bare(rcs(x, rcs_knots(x, nk))))
  }
})
...
test_that("predicting on new data reuses the fitted knots", {
  ...
  expect_error(rcs(nd$x, 4), class = "svyrcs_error")
})

test_that("invalid knot specifications are rejected with an svyrcs error", {
  set.seed(5)
  x <- rnorm(100)
  expect_error(rcs(x, 2), class = "svyrcs_error")
  expect_error(rcs(x, 8), class = "svyrcs_error")
  expect_error(rcs(x, 4.5), class = "svyrcs_error")
  expect_error(rcs(letters), class = "svyrcs_error")
  expect_error(rcs(x, c(1, 2)), class = "svyrcs_error")
  expect_error(rcs(x, c(1, 2, NA)), class = "svyrcs_error")
  expect_error(rcs(rep(1, 10), 4), class = "svyrcs_error")
  expect_error(rcs(rep(NA_real_, 10), 4), class = "svyrcs_error")
})
...
test_that("coefficient names are the term label with primes", {
  ...
  d$y <- rnorm(200)
  fit <- lm(y ~ rcs(x, 4) + z, data = d)
  expect_equal(names(coef(fit)),
               c("(Intercept)", "rcs(x, 4)", "rcs(x, 4)'", "rcs(x, 4)''", "z"))
})
